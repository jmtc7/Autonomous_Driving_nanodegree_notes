...
void printSamples(double gps_x, double gps_y, double theta)
{
  ...
  // DONE: Set standard deviations for x, y, and theta
  std_x = 2;
  std_y = 2;
  std_theta = 0.05;
  ...
  // DONE: Create normal distributions for y and theta
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    sample_x = dist_x(gen); //"gen" is the random engine initialized earlier
    sample_y = dist_y(gen);
