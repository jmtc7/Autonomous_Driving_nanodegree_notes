...
float motion_model(float pseudo_position, float movement, vector<float> priors, int map_size, int control_stdev)
{
  ...
  for(int pre_pseudo_pos=0; pre_pseudo_pos<map_size; pre_pseudo_pos++)
  {
      // Transition probability
      float value = pseudo_position-pre_pseudo_pos;
      float transition_prob = Helpers::normpdf(value, movement, control_stdev);
      
      // Belief in the previous state
      float belief_prev = priors[pre_pseudo_pos];
      
      // Add probability to the sum
      position_prob += transition_prob*belief_prev;
  }
